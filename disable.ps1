...
try {
    ...
    if ($userFound) {
        $action = 'Found'
        $dryRunMessage = "Disable Ultimo-User account for: [$($p.DisplayName)] will be executed during enforcement"
    } else {
        $action = 'NotFound'
    ...
    if (-not($dryRun -eq $true)) {
        switch ($action) {
            'Found' {
                Write-Verbose "Disable Ultimo-User account with accountReference: [$aRef]"
    ...
    $auditMessage = "Could not disable Ultimo-User account. Error: $($errorObj.FriendlyMessage)"
